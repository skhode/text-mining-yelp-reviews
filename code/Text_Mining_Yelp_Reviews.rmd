...
**Reading the Data**
```{r warning=FALSE,message=FALSE, error=FALSE}
data<-read_csv2("yelpRestaurantReviews_sample_dataset.csv")
```
...
xnew = x %>% mutate(atr= str_split(attributes,'\\|')) %>% unnest(atr)
#head(xnew)
dim(xnew)
n=dim(xnew)[1]

xnew3 = xnew %>% cbind(str_split_fixed(xnew$atr,":",2))
colnames(xnew3)[4] = 'Atrname'
colnames(xnew3)[5] = 'AtrVal'
xnew3 = xnew3 %>% select(-c(attributes,atr))
head(xnew3)
dim(xnew3)
#view(xnew3)

xnew4 = xnew3 %>% filter(xnew3$Atrname!='') %>% pivot_wider(names_from = Atrname, values_from = AtrVal)
#view(xnew4)
```

**Expanding the Ambience Attribute**
```{r}
xnew5 = xnew4 %>% mutate(Amb= str_split(Ambience,","))

extractamb = function(q)
  {
  sub(":.*","", q[which(str_extract(q, "True") == "True")])
  }

x6<-xnew5 %>% mutate( Amb= lapply( Amb, extractamb))

x6 %>% group_by(Amb) %>% tally() %>% view()

FinalData = df %>% merge(x6, on = review_id) %>% select(-Ambience)
x7 = FinalData %>% select(review_id,Amb)
x7 = x7 %>% unnest(Amb, keep_empty = TRUE) %>% mutate(Value = 1)
x7 = x7 %>% pivot_wider(names_from = Amb, values_from = Value)
colnames(x7) = c("review_id","Casual","Romantic","Classy","Trendy","Upscale","None","Divey","Intimate","Hipster","Touristy")
x7[is.na(x7)] = 0
x7$Casual = as.factor(as.logical(x7$Casual))
x7$Romantic = as.factor(as.logical(x7$Romantic))
x7$Classy = as.factor(as.logical(x7$Classy))
x7$Trendy = as.factor(as.logical(x7$Trendy))
x7$Upscale = as.factor(as.logical(x7$Upscale))
x7$None = as.factor(as.logical(x7$None))
x7$Divey = as.factor(as.logical(x7$Divey))
x7$Intimate = as.factor(as.logical(x7$Intimate))
x7$Hipster = as.factor(as.logical(x7$Hipster))
x7$Touristy = as.factor(as.logical(x7$Touristy))


FinalData = FinalData %>% left_join(x7) %>%  select(-Amb)
str(FinalData)

```

**Summarize Attributes by Restaurants & Star Ratings**
```{r}
#Alcohol
FinalData %>% group_by(Alcohol) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(Alcohol) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
```{r}
#Noise
FinalData %>% group_by(NoiseLevel) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(NoiseLevel) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
```{r}
#GoodforKids
FinalData %>% group_by(GoodForKids) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(GoodForKids) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
```{r}
#WiFi
FinalData %>% group_by(WiFi) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(WiFi) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
FinalData %>% group_by(RestaurantsPriceRange2) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(RestaurantsPriceRange2) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
```{r}
#Touristy
FinalData %>% group_by(Touristy) %>% distinct(business_id) %>% tally()
FinalData %>% group_by(Touristy) %>% summarise(num_ratings=n(), 
num_5_rat=round(sum(starsReview==5)*100/num_ratings),
num_4_rat=round(sum(starsReview==4)*100/num_ratings),
num_3_rat=round(sum(starsReview==3)*100/num_ratings),
num_2_rat=round(sum(starsReview==2)*100/num_ratings),
...
**Prepare Data for the Select Attributes**
```{r}
FinalData %>% mutate(hiLo = ifelse(starsReview <= 2, -1, ifelse(starsReview >=4, 1, 0 )))
FinalData1 = FinalData %>% select(-c(text,useful,funny,cool,name,neighborhood,address,city,state,postal_code,starsBusiness, review_count,is_open,attributes,categories,longitude,latitude,starsReview,date,hours,Review))
#glimpse(FinalData1)
FinalData1 = FinalData1 %>% select(review_id,GoodForKids,NoiseLevel,OutdoorSeating,RestaurantsPriceRange2,WiFi,DriveThru)
FinalData1$GoodForKids = as.factor(FinalData1$GoodForKids)
FinalData1$NoiseLevel = as.factor(FinalData1$NoiseLevel)
FinalData1$OutdoorSeating = as.factor(FinalData1$OutdoorSeating)
FinalData1$RestaurantsPriceRange2 = as.factor(FinalData1$RestaurantsPriceRange2)
FinalData1$WiFi= as.factor(FinalData1$WiFi)
FinalData1$DriveThru = as.factor(FinalData1$DriveThru)
FinalData1 = FinalData1 %>% left_join(x7)
...
**Prepare Data to be grouped by attributes**
```{r}
combined_trn2 = combined_trn[,"review_id"]
combined_trn2 = combined_trn2 %>% left_join(FinalData1)
combined_tst2 = combined_tst[,"review_id"]
combined_tst2 = combined_tst2 %>% left_join(FinalData1)
str(combined_trn2)
str(combined_tst2)
trainpred = predict(rfModel4, combined_trn1)
cmbtrainpred = trainpred$predictions
Trainset = cbind(combined_trn2,Actual = combined_trn$hiLo,Prediction = cmbtrainpred[,2]>0.4)
Trainset$Prediction = as.numeric(Trainset$Prediction)
Trainset = Trainset %>% mutate(Prediction = ifelse(Trainset$Prediction==0,-1,1))
testpred = predict(rfModel4, combined_tst1)
cmbtestpred = testpred$predictions
Testset = cbind(combined_tst2,Actual = combined_tst$hiLo,Prediction = cmbtestpred[,2]>0.4)
Testset$Prediction = as.numeric(Testset$Prediction)
Testset = Testset %>% mutate(Prediction = ifelse(Testset$Prediction<1,-1,1))

print(table(Trainset$Actual,Trainset$Prediction))
print(table(Testset$Actual,Testset$Prediction))
```

**Best Model(RF) Evaluation with attributes**
```{r}
#Good for Kids
Trainset %>% group_by(GoodForKids) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(GoodForKids) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```

```{r}
#Noiselevel
Trainset %>% group_by(NoiseLevel) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(NoiseLevel) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```

```{r}
#Outdoor Seating
Trainset %>% group_by(OutdoorSeating) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(OutdoorSeating) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```

```{r}
#Price Range
Trainset %>% group_by(RestaurantsPriceRange2) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(RestaurantsPriceRange2) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```

```{r}
#Wifi
Trainset %>% group_by(WiFi) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(WiFi) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```

```{r}
#Touristy
Trainset %>% group_by(Touristy) %>% summarise(Train_Count=n(),Accuracy = mean(Actual==Prediction))
Testset %>% group_by(Touristy) %>% summarise(Test_Count=n(),Accuracy = mean(Actual==Prediction))
```
